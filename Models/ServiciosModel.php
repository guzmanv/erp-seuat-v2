...
class ServiciosModel extends Mysql
{
    ...
    public function selectServicios()
    {
        ...
        $sql = "SELECT ts.id AS IdServicios, ts.codigo_servicio AS CodigoServicio, ts.nombre_servicio AS NombreServicio, ts.precio_unitario AS PrecioUnitario, ts.aplica_edo_cuenta AS AplicaEdoCuenta, ts.estatus AS EstatusServicios, tp.abreviacion_plantel AS Plantel, tp.municipio AS Municipio
                FROM t_servicios ts
                INNER JOIN t_planteles tp 
                    ON ts.id_plantel = tp.id
                INNER JOIN t_categoria_servicios tcs
                    ON ts.id_categoria_servicio = tcs.id 
                INNER JOIN t_unidades_medida tum 
                    ON ts.id_unidades_medida = tum.id
                WHERE ts.estatus !=0
                ORDER BY ts.nombre_servicio ASC ";
