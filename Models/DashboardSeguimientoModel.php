...
class DashboardSeguimientoModel extends Mysql{
    ...
    public function selectCards($plt)
    {
        if($plt == "all")
        {
            ...
            $sql3 = "SELECT COUNT(*) as total FROM (SELECT id_persona FROM t_seguimiento_prospecto GROUP BY id_persona) as total";
            $requestTotProspectos = $this->select($sql3);
            $request['totalProspectosSeguimiento'] = $requestTotProspectos['total'];
