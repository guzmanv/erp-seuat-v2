				...
				<div class="text-center">
					<div class="btn-group">
						...
						<div class="dropdown-menu">
							<button class="dropdown-item btn btn-outline-secondary btn-sm btn-flat icono-color-principal btnEditServicio" onClick="fntServicio(this,'.$arrData[$i]['IdServicios'].')" title="Editar"> &nbsp;&nbsp; <i class="fas fa-pencil-alt"></i> &nbsp; Editar</button>
		...
		public function setServicio()
		{
			...
			//if($_SESSION['permisosMod']['w']){
				if(empty($_POST['txtCodigo_servicio']) || empty($_POST['txtNombre_servicio']) || empty($_POST['txtPrecio_unitario']) || empty($_POST['listAnioFiscal']) || empty($_POST['listEstatus']) || empty($_POST['txtFecha_creacion']) || empty($_POST['txtId_usuario_creacion'])  || empty($_POST['listIdPlantel']) || empty($_POST['listIdCategoria_servicio']) || empty($_POST['listIdUnidades_medida']) )
					...
					if($intIdServicio == 0){
						$request_servicio = $this->model->insertServicio($strCodigo_servicio,
																		   $strNombre_servicio, 
																		   $intPrecio_unitario, 
																		   $intAplica_edo_cuenta, 
																		   $strAnio_fiscal, 
																		   $intEstatus, 
																		   $strFecha_creacion, 
																		   ...
																		   $intIdPlantel, 
																		   $intIdCategoria_servicio,
																		   $intIdUnidades_medida );
																		   $option = 1;
					} 
